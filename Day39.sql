...
SELECT * 
FROM amazon_products AS a
LEFT JOIN return_records AS r
...
SELECT a.category, ROUND(COUNT(r.return_id)::numeric/(SELECT COUNT(*) FROM return_records)::numeric *100,2) AS returns_by_category
FROM amazon_products AS a
LEFT JOIN return_records AS r
ON a.product_id = r.product_id
GROUP BY 1
