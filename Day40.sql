...
--Write a SQL query to analyze the order patterns throughout the day, providing insights into each hour's total orders and their respective percentages of the total orders. 
...
SELECT EXTRACT(HOUR FROM order_time) as hourly_data, COUNT(customer_id)as total_hourly_orders, ROUND(100*COUNT(customer_id)::numeric/(SELECT COUNT(*) from order_data)::numeric,2) AS order_percent_per_hr
FROM order_data
GROUP BY 1
ORDER BY 3 DESC
