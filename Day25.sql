...
INSERT INTO customers10 (id, first_name, last_name, city, address, phone_number) VALUES
...
INSERT INTO orders10 (id, cust_id, order_date, order_details, total_order_cost) VALUES
...
SELECT * 
FROM customers10 as c
RIGHT JOIN orders10 AS o
ON o.id = c.id

--You have given two tables customers with columns (id, name phoneaddress) and orders table columns(order_id, cxid order_date and cost)
--Find the percentage of shipable orders. Consider an order is shipable if the customer's address is known.

SELECT ROUND(100*(COUNT(c.id)::numeric/ 
      (
	     SELECT COUNT(*) 
		 FROM customers10 as c
         RIGHT JOIN orders10 AS o
